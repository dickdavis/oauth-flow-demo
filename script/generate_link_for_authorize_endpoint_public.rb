...
client = OAuth::Client.where(client_type: 'public').first
client_id = client.id
...
puts <<~TEXT
  ...
  http://localhost:3000/oauth/authorize?&client_id=#{client_id}&response_type=code&code_challenge=#{code_challenge}&code_challenge_method=S256&redirect_uri=#{client.redirect_uri}
